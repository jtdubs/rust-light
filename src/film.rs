...
impl<'a> Film<'a> {
    pub fn new(width : u32, height : u32, f : &'a Filter + 'a) -> Film<'a> {
        Film { width: width, height: height, filter: f, pixels: Vec::<Pixel>::from_fn((width * height) as uint, |_| Pixel::new()) }
    }

    pub fn new_1080(f : &'a Filter + 'a) -> Film<'a> { Film::new(1920, 1080, f) }
    pub fn new_720(f : &'a Filter + 'a) -> Film<'a> { Film::new(1280, 720, f) }
    pub fn new_480(f : &'a Filter + 'a) -> Film<'a> { Film::new(720, 480, f) }
    pub fn new_2k(f : &'a Filter + 'a) -> Film<'a> { Film::new(2048, 1080, f) }
    pub fn new_4k(f : &'a Filter + 'a) -> Film<'a> { Film::new(4096, 2160, f) }
    pub fn new_8k(f : &'a Filter + 'a) -> Film<'a> { Film::new(8192, 4608, f) }
    pub fn new_qvga(f : &'a Filter + 'a) -> Film<'a> { Film::new(320, 240, f) }
    pub fn new_vga(f : &'a Filter + 'a) -> Film<'a> { Film::new(640, 480, f) }
